...
export function RubroTable({ rubros, onEdit, onAdd, onDelete }: RubroTableProps) {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(r => r.nombre.toLowerCase().includes(lowSearch));
    }

    if (sortConfig.key && sortConfig.direction) {
      result.sort((a, b) => {
        const valA = a[sortConfig.key as keyof Rubro] || '';
        const valB = b[sortConfig.key as keyof Rubro] || '';
